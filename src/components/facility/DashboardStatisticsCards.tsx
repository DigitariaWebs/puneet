...
import { Building, Users, UserCheck, MapPin } from "lucide-react";
import { facilities } from "@/data/facilities";

export function DashboardStatisticsCards() {
  const totalFacilities = facilities.length;
  const totalUsers = facilities.reduce((sum, f) => sum + f.usersList.length, 0);
  const totalActiveClients = facilities.reduce(
    (sum, f) => sum + f.clients.filter((c) => c.status === "active").length,
    0,
  );
  const totalLocations = facilities.reduce(
    (sum, f) => sum + f.locationsList.length,
    0,
  );
